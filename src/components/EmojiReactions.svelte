<script>
    ...
    $: emojiColumnMap = emojis.reduce((acc, emoji, i) => {
        const key = emoji.includes('❤') ? '❤' : emoji;
        acc[key] = `emoji${i + 1}`;
        return acc;
    }, {});
    ...
    async function handleReaction(emoji) {
        try {
            ...
            if (oldSelection) {
                const key = oldSelection.includes('❤') ? '❤' : oldSelection;
                const col = emojiColumnMap[key];
                if (col) reactionCounts[col] = Math.max(0, reactionCounts[col] - 1);
            }
            if (newSelection) {
                const key = newSelection.includes('❤') ? '❤' : newSelection;
                const col = emojiColumnMap[key];
        ...
        <div class="emoji-options">
            {#each availableEmojis as emoji, i}
                <button
                    ...
                    title={`React with ${emoji} (${reactionCounts[emojiColumnMap[emoji.includes('❤') ? '❤' : emoji]] || 0})`}
                    style={`--i: ${i};`}
                >
                    <span class="emoji-icon">{emoji}</span>
                    {#if (reactionCounts[emojiColumnMap[emoji.includes('❤') ? '❤' : emoji]] || 0) > 0}
                        <span class="emoji-count">{reactionCounts[emojiColumnMap[emoji.includes('❤') ? '❤' : emoji]]}</span>
                    {/if}
                </button>
            {/each}
        </div>
